...
configure_ufw(){ [ "${ENABLE_UFW}" = "yes" ] || return 0; ufw allow OpenSSH; ufw allow 80/tcp; ufw allow 443/tcp; [ "${INSTALL_NPM}" = "yes" ] && ufw allow 81/tcp || true; ufw --force enable; }
configure_fail2ban(){ [ "${ENABLE_FAIL2BAN}" = "yes" ] || return 0; systemctl enable fail2ban; systemctl restart fail2ban; }
...
create_networks(){ docker network inspect "${PROXY_NETWORK}" >/dev/null 2>&1 || docker network create "${PROXY_NETWORK}"; docker network inspect "${DB_NETWORK}" >/dev/null 2>&1 || docker network create "${DB_NETWORK}"; }
prepare_redis_conf(){ local redis_dir="${VPS_ROOT}/vps-db/redis"; [ -f "${redis_dir}/redis.conf.example" ] && [ ! -f "${redis_dir}/redis.conf" ] && cp "${redis_dir}/redis.conf.example" "${redis_dir}/redis.conf" || true; }
